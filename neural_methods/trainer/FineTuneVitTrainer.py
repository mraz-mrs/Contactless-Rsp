...
class FineTuneVitTrainer:
    def __init__(self, configs):
        ...
        if not configs.TRAIN.PRETRAIN: 
            if configs.MODEL.PRETRAINED:
                checkpoint = torch.load(configs.MODEL.PRETRAINED, map_location='cpu')
            else:
                raise ValueError("Pretrained model path is required when not pretraining")
            print("Load pre-trained checkpoint from: %s" % configs.MODEL.PRETRAINED)
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            ...
            for idx, (val_stmap, val_rsp_label) in enumerate(tbar):
                ...
                rsp_pred, _ = self.model(val_stmap)
